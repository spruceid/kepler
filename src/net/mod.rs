...
/// TODO
// take libp2p-bitswap
//      it has a nicer API and takes care of more aspects w.r.t. auto-storage of recieved blocks
// asyncify OR channelify BitswapStore trait
// take Repo (BlockStore, DataStore and PinStore) trait(s) from ipfs-rust
//      use to impl BitswapStore trait either with async or channels
//      use in Bitswap
// look at inject_event for handling kepler-specific logic
//      filtering incoming connections by authorisation
//      authorisation handshake protocol (??)
// think about garbage collection for Repo??
// combine rust-ipfs::Repo with Bitswap and the ipfs-embed Behaviour stuff
...
impl<P: StoreParams> NetworkBehaviourEventProcess<()> for NetworkBackendBehaviour<P> {
